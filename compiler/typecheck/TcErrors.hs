...
mkHoleError _ctxt ct@(CHoleCan { cc_hole = ExprHole (OutOfScope occ rdr_env0) })
  ...
  where
    ...
    mk_bind_scope_msg bind_loc th_loc
      = hang (quotes (ppr occ) <+> bind_rng <+> text "is in scope")
           2 (rel_pos <+> text "the splice on" <+> th_rng)
      where
        bind_rng
          | is_th_bind = empty
          | otherwise  = parens (text "line" <+> int bind_ln)
        rel_pos
          | is_th_bind = text "beginning with"
        ...
        is_th_bind = bind_loc `isRealSubspanOf` th_loc
        isRealSubspanOf = isSubspanOf `on` RealSrcSpan
