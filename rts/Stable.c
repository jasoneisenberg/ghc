...
/* Each time the stable pointer table is enlarged, we retain the old version to
 * ensure dereferences are thread-safe (see Note [Enlarging the stable pointer
 * table]).  Since we double the size of the table each time, we can
 * (theoretically) enlarge it at most N times on an N-bit machine.  Thus, there
 * will never be more than N old versions of the table.
 */
#if SIZEOF_VOID_P == 4
 ...
 * existing entries, and then temporarily add the old version of the table to
 * the old_SPTs array.  By not immediately freeing the old version (or
 * equivalently by not growing the table using realloc()), we ensure that
 ...
 * #10296).  Finally, during the next GC run, we free all of the accumulated
 * old versions.
 *
 * Note that because the stable pointer table is doubled in size each time it is
 * enlarged, the total memory needed to store the accumulated old versions is
 * always less than that required to hold the current version.
 */
...
markStableTables(evac_fn evac, void *user)
{
     ...
     * is safe to free any accumulated old tables.
     */
    freeOldSPTs();
